...
import { Home, Map, Receipt, ShieldCheck, Inbox, Briefcase, MessageSquare, User, Users } from 'lucide-react';
...
export const MobileNav: React.FC<MobileNavProps> = ({ activeView, onNavigate, onOpenChat }) => {
  const { role } = useAuth();
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-40 bg-[#004331] text-white border-t border-emerald-800 shadow-2xl py-2 px-3 flex items-center justify-around lg:hidden pb-safe">
      {items.map((item) => {
